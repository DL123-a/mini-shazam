...
class HashTable:
    ...
    def lookup(self, key):
        ...
        """
        index = self._hash(key)
        return [value for (k, value) in self._buckets[index] if k == key]
    ...
    def stats(self):
        ...
        return {
            ...
            "avg_chain_length": round(sum(len(bucket) for bucket in self._buckets if bucket) / len([bucket for bucket in self._buckets if bucket]), 4)
        }
    ...
    def _next_prime(n):
        ...
        for i in range(3, int(math.floor(math.sqrt(candidate)))):
            if candidate % i == 0:
                candidate += 2
                break
